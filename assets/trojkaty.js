...
// trojkatyToFixed = false dla właściwego formatowania miejsc po przecinku (max 2)
let trojkatyToFixed = true;
function formatNumber(num) {
  if (trojkatyToFixed) return num.toFixed(2);
...
function updateMaxXaXb() {
  ...
  Xa.max = allowHalf ? lengthInput.value / 2 : lengthInput.value / 2 - 0.01;
  Xb.max = allowHalf ? lengthInput.value / 2 : lengthInput.value / 2 - 0.01;
}
...
function checkXaXbValue() {
  ...
  const XaPosition = (parseFloat(Xa.value) / lengthInput.value) * belkaWidth;
  const XbPosition =
    belkaWidth - (parseFloat(Xb.value) / lengthInput.value) * belkaWidth;
...
function updateTriangleContents() {
  ...
  document
    .querySelector(".tA")
    .style.setProperty("--contentA", `"${formatNumber(Xa)}"`);
  document
    .querySelector(".tB")
    .style.setProperty("--contentB", `"${formatNumber(Xb)}"`);
}
...
document.addEventListener("keydown", function (event) {
  if (event.key === "q" || event.key === "Q")
    Swal.mixin({
      ...
      icon: "info",
      title: `Ilość wykonanych obliczeń: ${iloscDzialan}`,
    });
});
